...
class BCE(Loss):
    ...
    def forward(self, y_out, y_truth, individual_losses=False):
        ...
        if individual_losses:
            ...
            # result = (-1/size_N) * (np.dot(y_truth, np.log(y_out)) + np.dot((1-y_truth), np.log(1-y_out)))
            result = (-1/size_N) * (np.dot(y_truth.T, np.log(y_out)) + np.dot((1-y_truth).T, np.log(1-y_out)))
    ...
    def backward(self, y_out, y_truth):
        ...
        gradient = None
        gradient = (-1/size_N)*((y_truth / y_out) - (1-y_truth)/(1-y_out))
        ########################################################################
        # TODO:                                                                #
