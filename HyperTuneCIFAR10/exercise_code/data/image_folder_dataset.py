...
class RandomRotationTransform:
    ...
    def __init__(self, degrees=15):
        """
        :param degrees: range of degrees to randomly rotate the image
        """
    ...
    def __call__(self, image):
        ...
        # Apply rotation using affine transform
...
class RandomCropTransform:
    ...
    def __call__(self, image):
        ...
        # Randomly determine the crop size (between 50% and 100% of the original size)
        crop_size_h = np.random.randint(h // 2, h + 1)
        crop_size_w = np.random.randint(w // 2, w + 1)

        # Calculate the cropping region without padding
        top = np.random.randint(0, h - crop_size_h + 1)
        left = np.random.randint(0, w - crop_size_w + 1)
...
class ColorJitterTransform:
    ...
    def __init__(self, brightness=0.2, contrast=0.2):
        """
        :param brightness: factor to adjust brightness
        :param contrast: factor to adjust contrast
        """
        self.brightness = brightness
        self.contrast = contrast

    def __call__(self, image):
        # Ensure the image is in the float32 range [0, 1]
        ...
        # Adjust brightness
        image = np.clip(image + self.brightness, 0.0, 1.0)
